...
function main_track_states.idle.transition(this, context, input)
    ...
    if (input == INPUT_BOLT) then
        ...
        if (this.main_track_states.shootmode == 0) then
            context:runAnimation("bolt_left", track, true, PLAY_ONCE_STOP, 0.2)
            this.main_track_states.shootmode = 1
        else
            context:runAnimation("bolt_right", track, true, PLAY_ONCE_STOP, 0.2)
...
function gun_kick_state.transition(this, context, input)
    ...
    if (input == INPUT_SHOOT) then
        ...
        if (this.main_track_states.shootmode == 0) then
            ...
            context:runAnimation("bolt_left", track, true, PLAY_ONCE_STOP, 0.2)
            this.main_track_states.shootleft = context:getCurrentTimestamp()
            if (context:getCurrentTimestamp() - this.main_track_states.shootright < get_bolt_time(context)) then
                context:adjustClientShootInterval(get_bolt_time(context) - (context:getCurrentTimestamp() - this.main_track_states.shootright))
            end
            this.main_track_states.shootmode = 1
        else
            context:runAnimation("shoot_right", track, true, PLAY_ONCE_STOP, 0)
            context:runAnimation("bolt_right", track, true, PLAY_ONCE_STOP, 0.2)
            this.main_track_states.shootright = context:getCurrentTimestamp()
            if (context:getCurrentTimestamp() - this.main_track_states.shootleft < get_bolt_time(context)) then
                context:adjustClientShootInterval(get_bolt_time(context) - (context:getCurrentTimestamp() - this.main_track_states.shootleft))
            end
            this.main_track_states.shootmode = 0
        end
    end
