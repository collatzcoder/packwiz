...
local MELEE_TRACK = increment(blending_track_top)
...
function gun_kick_state.transition(this, context, input)
    if (input == INPUT_SHOOT) then
        ...
        if (current_timestamp - last_shoot_timestamp > shoot_interval + 500) then
            ...
            gun_kick_state.melee_heavy_count = 0
        end
        -- 初始化射速计数器
        if (context:getShootInterval() > 510 or context:getShootInterval() < 490) then
            context:adjustClientShootInterval(500-context:getShootInterval())
        end
        local track = context:getTrack(BLENDING_TRACK_LINE, MELEE_TRACK)
        -- 播动画
        if (context:isAiming()) then
            -- 重击
            if (gun_kick_state.melee_heavy_count == 0) then
                context:runAnimation("melee_heavy_1", track, true, PLAY_ONCE_STOP, 0.2)
                context:adjustClientShootInterval(300)
                gun_kick_state.melee_heavy_count = 1
                gun_kick_state.melee_light_count = 0
            elseif (gun_kick_state.melee_heavy_count == 1) then
                context:runAnimation("melee_heavy_2", track, true, PLAY_ONCE_STOP, 0.2)
                context:adjustClientShootInterval(2000)
                gun_kick_state.melee_heavy_count = 0
                gun_kick_state.melee_light_count = 0
            end
        else
            ...
            elseif (gun_kick_state.melee_light_count == 1) then
                context:runAnimation("melee_light_1_1", track, true, PLAY_ONCE_STOP, 0.2)
                gun_kick_state.melee_light_count = 2
                -- gun_kick_state.melee_heavy_count = 0
